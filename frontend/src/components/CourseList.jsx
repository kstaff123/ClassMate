...
export function CourseList({ courses, variant }) {
  ...
  const containerStyles =
    variant === "cart"
      ? "flex flex-col bg-blue-400 rounded-lg p-3 text-white my-3 shadow-md"
          ...
          <li key={course.id} className={containerStyles}>
            <div className="font-medium">
              <div className="flex items-center justify-between">
                ...
                <div onClick={() => handleCartAction(course)} className="cursor-pointer">
                  ...
                  <div key={index} className="flex items-center font-normal text-xs sm:text-sm space-x-2">
                    ...
                    <p>
                      {schedule.start_time && schedule.end_time
                        ? `${convert24hourTo12hour(schedule.start_time)} - ${convert24hourTo12hour(schedule.end_time)}`
                        : "TBA"}
                    </p>
                  </div>
                ))
